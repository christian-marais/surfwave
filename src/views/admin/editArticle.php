    ...
    <div class="row">
      <form class="col-8 edit" method="POST" enctype="multipart/form-data">
        ...
        <div class="col-md-3 mb-3">
          ...
          <select name="catProd" class="custom-select" id="categorie" required>
            <?php foreach($categories as $categorie){
              echo'
              <option'?>
              <?=($categorie['categoProd']==$article['catProd'])?"selected":"";?>
        ...
        <div class="col-md-4 mb-3">
          ...
          <input name="puProd" class="form-control input" id="prix" value="<?=$article['puProd']?>"/>
        </div>
        ...
        <div class="mb-3">
          <h3>
            Changer l'image du produit (.jpg &lt;100ko)
          </h3>
          <img src="<?= HTTPS.'://'.$_SERVER['HTTP_HOST']?>/images/produits/<?= substr("000000",strlen($article['idProd'])).$article['idProd']?>.jpg" alt="<?= $article['nomImage']?>" class="img-fluid image">
          <?php
            if(!empty($_FILES)){
